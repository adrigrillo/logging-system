...
@Plugin(name = "JSON", category = Node.CATEGORY, elementType = Layout.ELEMENT_TYPE, printObject = true)
public final class JSON extends AbstractStringLayout {
	...
	static final SimpleDateFormat isoDateFormat = new SimpleDateFormat(TIMESTAMP_FORMAT);
	...
	protected JSON(final Configuration config, final boolean locationInfo, final boolean properties,
		...
		super(config, charset, //
				PatternLayout.newSerializerBuilder().setConfiguration(config).setReplace(null).setPattern(headerPattern)
						.setDefaultPattern(DEFAULT_HEADER).setPatternSelector(null).setAlwaysWriteExceptions(false)
						.setNoConsoleNoAnsi(false).build(),
				PatternLayout.newSerializerBuilder().setConfiguration(config).setReplace(null).setPattern(footerPattern)
						.setDefaultPattern(DEFAULT_FOOTER).setPatternSelector(null).setAlwaysWriteExceptions(false)
						.setNoConsoleNoAnsi(false).build());
	...
	private static LogEvent convertMutableToLog4jEvent(final LogEvent event) {
		// TODO (from JsonLayout): Need to set up the same filters for MutableLogEvent
		// but don't know how...
	...
	private final String format(final LogEvent event) {
		...
		json(sb, "timestamp", isoDateFormat.format(event.getTimeMillis()));
		...
		// Message
		json(sb, "message", event.getMessage().getFormattedMessage());
		// Exceptions
	...
	@PluginFactory
	public static JSON createLayout( //
		...
		return new JSON(config, locationInfo, properties, complete, eventEol, headerPattern, footerPattern,
